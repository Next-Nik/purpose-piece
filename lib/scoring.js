...
function createSession() {
  return {
    ...
    recognitionAttempts: 0,   // tracks correction loop depth
...
function getTopArchetypes(session) {
  ...
  // Flag close Maker/Architect scores for pressure fork
  const makerScore     = session.archetypeTally.maker     || 0;
  const architectScore = session.archetypeTally.architect || 0;
  const needsMakerArchitectFork = (
    Math.abs(makerScore - architectScore) <= 1 &&
    (makerScore > 0 || architectScore > 0) &&
    (makerScore + architectScore >= 2)
  );
