...
function answer(session, rawInput) {
  ...
  if (session.phase === 1) {
    ...
    // Extract letter — handles "I think G" or "probably b" etc.
    const letterMatch = lower.match(/\b([a-g])\b/);
    ...
    // Detect if answer contains multiple letter candidates (hedging)
    const allLetters = lower.match(/\b[a-g]\b/g) || [];
