...
function answer(session, userAnswer) {
  ...
  if (session.phase === 1) {
    ...
    // Extract letter even from natural answers like "I think B" or "probably c"
    const letterMatch = lower.match(/\b([a-f])\b/);
    const selectedOption = question && letterMatch &&
      question.options.find(opt => opt.id === letterMatch[1]);
    ...
    return { type: "question", question: nextQuestion, acknowledgment: null };
  }
  ...
  if (session.phase === "2-behavior") {
    ...
    // Detect rich/interesting answers to warrant a reaction
  ...
  if (session.phase === "2-scale") {
    // Extract letter from natural answer
    const letterMatch = lower.match(/\b([a-d])\b/);
    const scaleQuestion = questions.PHASE_2_QUESTIONS.scale;
    const selectedOption = letterMatch &&
      scaleQuestion.options.find(opt => opt.id === letterMatch[1]);
  ...
  if (session.phase === "2-domain-needed") {
    ...
    // If they say "all of them" or express breadth, note it and default to vision
    const expressedBreadth = lower.includes("all") || lower.includes("everything") || lower.includes("umbrella") || lower.includes("across");
    session.expressedBreadth = expressedBreadth;
    session.domain = matchedDomain || (expressedBreadth ? "vision" : "society");
    session.phase = "2-subdomain";
    return askSubdomainQuestion(session, expressedBreadth);
  }
  ...
  if (session.phase === "2-subdomain") {
    ...
    const selectedOption = subdomainMenu && letterMatch &&
      subdomainMenu.options.find((opt, idx) =>
        String.fromCharCode(97 + idx) === letterMatch[1]
      );
  ...
  if (session.phase === 3) {
    ...
    if (session.recognitionStep === 1) {
      // Read sentiment broadly — not just yes/no
      const positive = lower.includes("yes") || lower.includes("that fits") ||
        lower.includes("accurate") || lower.includes("true") || lower.includes("right") ||
        lower.includes("exactly") || lower.includes("definitely") || lower.includes("absolutely");

      const uncertain = lower.includes("think so") || lower.includes("kind of") ||
        lower.includes("sort of") || lower.includes("mostly") || lower.includes("trying") ||
        lower.includes("not yet") || lower.includes("not fully") || lower.includes("want to") ||
        lower.includes("hope") || lower.includes("working toward");

      const negative = lower.includes("no") || lower.includes("doesn't fit") ||
        lower.includes("not really") || lower.includes("off") || lower.includes("wrong");

      if (positive) {
        session.recognitionStep = 2;
        return { type: "recognition_step_2", worldImpact: scoring.generateWorldImpact(session.primaryArchetype) };
      } else if (uncertain) {
        // Hold the gap — this is important data, not a failed answer
        session.recognitionStep = 2;
        return { type: "recognition_gap", archetype: session.primaryArchetype };
      } else if (negative) {
        return { type: "correction_needed", text: "What part doesn't feel right?" };
      } else {
        // Treat ambiguous as uncertain rather than demanding binary
        session.recognitionStep = 2;
        return { type: "recognition_gap", archetype: session.primaryArchetype };
      }
    }
    ...
    if (session.recognitionStep === 2) {
      const positive = lower.includes("yes") || lower.includes("that fits") ||
        lower.includes("makes sense") || lower.includes("true") || lower.includes("right") ||
        lower.includes("exactly") || lower.includes("trying") || lower.includes("want") ||
        lower.includes("hope") || lower.includes("think so") || lower.includes("kind of") ||
        lower.includes("working") || lower.includes("becoming");
      ...
      if (positive) {
        ...
        return { type: "correction_needed", text: "What part doesn't feel right? Let's make sure this actually fits." };
      }
    }
...
function askSubdomainQuestion(session, expressedBreadth) {
  const menu = questions.SUBDOMAIN_MENUS[session.domain];
  if (!menu) {
    // Fallback if domain menu missing
    session.phase = 3;
    session.recognitionStep = 1;
    return {
      type: "recognition_step_1",
  ...
  if (expressedBreadth) {
    acknowledgment = "That's a useful frame — someone building the container others work inside. Let's find where your energy sits within it.";
  }
